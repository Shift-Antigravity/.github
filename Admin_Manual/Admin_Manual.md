# 室長用マニュアル

## 1. 機能概要

### 1-1. 概要

シフト管理を容易にするシステムです。機能は次の通りです。

- 翌月分のシフトの集計
- 塾マネに自動で反映

### 1-2. 動作環境

現在下記の動作環境は確認済みです。対応してほしいブラウザ等ございましたらお申し付けください。

- webアプリケーション
    - Google Chrome
    - firefox

- 拡張機能
    - Google Chrome

### 1-3. 拡張機能のインストール

現在個人で開発をしているためストアでの公開を行っておりません。そのため個人で対応させて頂きます。

## 2. ログイン

まず講師用のログイン画面を開きます。

![TutorLogin](./img/Tutor_login.png)

下側`室長(管理者)ログインはこちら`を押下することで管理者用のログイン画面に遷移します。

![AdminLogin](./img/Admin_login.png)

画面遷移後ホーム画面が表示されます。

![AdminHome](./img/Admin_home.png)

それぞれの機能概は次の通りです。

| 品目 | 配置 | 機能 |
| ----- | --- | ------- |
| シフト期間管理 | 上段左 | シフト発行 |
| 講師管理 | 上段中央 | 講師の登録・削除、レギュラーコマの承認やパスワードリセット |
| 各種設定 | 上段右 | 講師登録時の招待コードの変更、授業時間の設定 |
| 募集中のシフト期間 | 中段 | 募集しているシフトの集計、提出状況を見ることが出来ます |
| 過去データの管理・アーカイブを開く | 下段 | 過去募集したシフトの参照・削除が出来ます |

> - 3/24更新 募集中のシフト期間についてで募集しているシフトが無ければ最新の募集したシフトを取得しています。

## 3. シフト発行

ホーム画面上段左`シフトを発行する`を押下します。発行設定画面に遷移します。

![ShiftIssuance](./img/Shift_issuance.png)

### 3-1. 対象月・提出期限の設定

まず画面上部で募集する対象月、締め切りを設定します。
画像の設定では`2026年8月`のシフトを`2026/7/14 23:59`締め切りに設定をしています。

![ShiftSettingDeadline](./img/ShiftSettingDeadline.png)

### 3-2. 塾の日程を追加

模試や定期テストが近いなどの講師に把握してほしい予定がある場合に有効です。対象の日にちを選択して入力することで可能です。

> - 入力は一件までなので句読点で区切るなど工夫をお願いします。
> - 日にちを跨いでの登録は実装していません。要望があれば実装します。

![ShiftEventSetting](./img/ShiftEventSetting.png)

### 3-3. 講師への連絡

画面右側黒色の画面で講師個人に連絡をすることが出来ます。

### 3-4. 発行

`シフトを発行して公開`を押下して完了です。

### 3-5. 共有

ログイン用のURL `https://shiftantigravity.web.app/login`を共有すれば新規登録・集計出来ます。新人講師を追加するときは例えば[URLをQRコードに変換するwebサイト](https://qr.quel.jp/url.php)などを用いてURLを変換・印刷を行い、面接時などに共有するという運用を想定しています。

> `3-4. 発行`までの手順を行っても講師側には通知が行きません。そのためLINEなどを用いて上記URLを共有してください。

URLをQRコードに変換するwebサイト例

- `https://qr.quel.jp/url.php`
- `https://www.cman.jp/QRcode/qr_url/`

## 4. 集計

提出状況の確認のやり方です。

![AdminHome2](./img/Admin_home2.png)

上記画像の下段に先ほど登録したシフトが反映されます。

### 4-1. 提出状況を確認

`提出状況を確認`を押下して提出状況を確認出来ます。

![SubmitStatus](./img/Submit_status.png)

また講師名で検索、未提出者のみ表示が出来ます。

### 4-2. 集計確認

`集計モードで開く`またはホーム画面での`提出状況を確認`の右側にあるアイコンを押下します。すると次のような画面に遷移します。

![CollectShift](./img/Collect_shift.png)

提出された内容を確認できます。また左右の矢印キーで移動が可能です。塾マネを開きながら見比べる使い方を想定しています。

> - リマインドは実装していません。そのため押下しても機能しません。
> - 3/24更新 レギュラーコマに黄色のマークを付けました。より欠勤日の視認性を向上しています。

## 5. 自動入力

塾マネに提出されたシフトを反映します。塾マネで対象の講師の`複数コマ出勤登録`の画面を開きます。
またGoogle Chrome拡張機能も開いてください。

![JukuManeExtension](./img/JukuMane_Extension.png)

> 開いたときに講師名と年月を取得するので自動で検索欄に講師名が入ります。もし見つからない場合は名前を検索してください。

`データ取得・照合`をクリックしてください。そうすると`講師からの連絡`や右上に緑色で`26件取得しました`と表示されているのが分かります。

![JukuManeGet](./img/JukuMane_Get.png)

そして`システムへ反映`を押下することで次のような画面になりOKを押下して反映されます。

![JukuMane_ref](./img/JukuMane_ref.png)

![JukuMane_AutoFill](./img/JukuMane_AutoFill.png)

最後に塾マネの`登録`を押して完了です。

> 2026/3/11 更新
>> 現在拡張機能を開いた際にログインを要求しています。ユーザー名パスワードは共有された室長用のアカウントと同じです。
>> また拡張機能の動作やUIを一部修正しているため見た目は上記とは異なります。しかし基本的には同じ操作で利用できます。

## 6. 講師の管理

ホーム画面上段中央、緑色の`講師一覧を確認`から次のような画面に遷移します。

![ManageTutor](./img/Manage_Tutor.png)

主な機能は次の通りです。

- 設定
- パスワードリセット
- 削除

### 6-1. 設定

黒の`設定`ボタンを押下しすると次のような画面に遷移します。

![TutorSetting](./img/Tutor_setting.png)

この画面ではレギュラーコマの管理、メモの追加が出来ます。

### 6-2. パスワードリセット

パスワードを忘れた人のための機能です。オレンジ色のボタンをクリックし、OKを押下すると新しくパスワードを入力するフォームが現れるのでそこで再設定してください。

### 6-3. 削除

赤色ゴミ箱のボタンで退職された講師の削除を行えます。

## 7. レギュラーコマの変更・承認

レギュラーコマの変更の承認を行います。もしリクエストがある場合画像のように赤色で件数が表示さます。

![Regular_Change](./img/Regular_Change.png)

このまま`レギュラー変更申請の確認`をクリックしてください。

そうすると希望日時と変更理由を確認出来ます。

![Regular_request](./img/Regular_request.png)

名前左のチェックボックスにチェックを入れ、承認・却下をしてください。

## 8. その他設定

ホーム画面上段右`各種設定`をクリックすると次の画面に遷移します。

![Admin_setting](./img/Admin_setting.png)

この画面では招待コードや時間割の変更が出来ます。

> `招待コード`で校舎の区別しています。そのため他校舎と同じ招待コードを設定するとエラーになります。
